...
- **语义拟物（semantic）**：桌面、文件夹、回收站、剪切板——用旧世界的名词给数字对象命名分类。
...
![课程表总览：从陌生机器到撤除脚手架（本文绘制）](assets/fig_gui_curriculum.png)

**第一课：把机器的概念翻译成人的语言。** Xerox Star（1981）的革命性不在于"图标画得像"，而在于它改变了你面对计算机时的基本词汇：你面对的不再是程序、进程、存储结构，而是文档、文件夹、收/发件篮（in/out basket）、打印机。
...
**第二课：GUI 语法标准化。** 早期的 Windows 1.0（1985）则更像 DOS 的视觉控制层；到 Windows 3.x，用户需要同时理解 Program Manager 和 File Manager 两套概念——"程序在哪里"和"文件在哪里"是两个问题。今天看来自然的 app/file 区分，曾经是必须被明确教授的知识。
...
**第三课：系统状态必须可见。**Windows 95 的设计团队（Kent Sullivan 在 CHI 1996 上公开了完整的过程）通过易用性测试发现，新手真正的困难不是"找不到命令"，而是**系统状态不可见**：最小化一个窗口之后，那个任务仿佛消失了。任务栏的价值在于让"仍然存在的任务"持续可见；开始按钮则是把整个系统收拢成一处随时可以返回的起点。
...
这里有一个鲜为人知的细节，我认为是整个 UI 史上最有教育意义的失败案例之一：Windows 团队做过一个 **Beginner Shell**：一个为新手极度简化的图形界面，最后却**主动放弃了**。原因有三：功能不全，用户迟早要退出；在简化界面里学到的东西迁移不到标准界面；等于要学两套系统。
...
**第五课：完成教育，撤出脚手架。** Metro（2011）把 "原生数字化"（authentically digital）和 "重内容，轻界面"（content not chrome） 写进官方设计理念，明确反对为了传统而模拟现实控件。iOS 7 常被讲成"去拟物"，但这个说法不准确：它减弱的是物件层面的视觉写实（皮革、木纹），同时**增强了**运动、层级和半透明这类空间与行为的写实。真正的变化是：交互语法的延续让这次换代几乎没有增加学习成本（UIKit 结构没动，换的是皮肤），于是显式脚手架可以撤掉一部分。
...
Material Design（2014）随后做了一个不一样的转变：设计团队用**真实纸张**做实验，观察光影和纸片的运动，再抽象成规则。这不是 iOS 6 式的拟物，不是假装 Gmail 是一个皮革信封，而是从"模仿具体物件"转向"模拟物理规律"。
...
Fluent Design 的 Acrylic/Mica 同理：质感回来，但它承载的是层级、焦点和上下文，而不是"这是真的玻璃"。

这里必须插入两个限定，否则上面的叙事就成了"历史注定朝这个方向走"的故事：
...
**钱。** 物物交换需要"双重欲望巧合"（你想要对方手里的东西，对方也恰好想要你手里的）而钱把劳动、债务、信用、承诺这些高维社会关系，压缩成一个双方都可操作的代币（token）：10 块钱。Giddens 把货币称作象征标志（symbolic token）：它可以在完全不依赖持有者身份的情况下跨时空流通。你不需要认识铸币厂厂长，也敢收这张钞票。
...
**公司。** 先看一个"不存在的人"：它可以签合同、拥有财产、被告上法庭。公司就是这样——一群人、资产、流程和合同的集合，法律却把它当作一个持续的、可识别的主体，**一个不需要"一张脸"的社会行动者**。Blair（2013）和 Pollman（2021）在公司法文献里详细梳理了这种法律人格如何实现连续性、资产分割和内部治理。社会早就接受了一个没有身体、没有脸、但持续存在并行动的主体。
